...
var logsCmd = &cobra.Command{
	Use:   "logs",
	Short: "View reverse logs",
	Long:  `View the logs generated by Reverse. This command allows you to see the log output for debugging and monitoring.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if dataDir == "" {
			dataDir = filepath.Join(os.TempDir(), "reverse")
		}
		if cwd == "" {
			cwd, _ = os.Getwd()
		}

		logsFile := filepath.Join(dataDir, "logs", "reverse.log")
		_, err = os.Stat(logsFile)
		if os.IsNotExist(err) {
			log.Warn("No logs found. Logs will be created when reverse runs with logging enabled.")
			return nil
		}
